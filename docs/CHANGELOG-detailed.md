# Changelog (Detailed History · 详细历史)

> 这是详细技术变更日志（中文 · 含 Bug 三段式与设计决策）。
> For brief bilingual highlights, see [CHANGELOG.md](../CHANGELOG.md).
...
**场景**：海外用户切到英文后点击 ⌨️ 帮助按钮，期望看到「Sidebar / View / Navigation / Help」。

**之前**：弹出的快捷键面板**整段中文**。同时切换语言后已经打开的时光机、历史抽屉、最近列表也保持原语言不变，必须刷新页面才能更新。

**现在**：
- ⌨️ 快捷键面板：6 段标题 + 9 条快捷键说明 + 底部 tip，全部跟语言切换
- 时光机快捷条「↩ Time machine: 5 min ago / 10 min ago / 30 min ago / 1 hour ago / 1 day ago」
- 历史项卡片「vs current / +N lines / -N lines / 👁 Preview / ↩ Restore / Auto-snapshot / Pre-overwrite backup / Pre-restore backup」
- 收藏 / 最近列表的标题、空态、清空按钮、× 按钮 tooltip
- 树空态「No scan directories yet / ＋ Add directory」
- 拖拽移动 confirm「Move directory "X" to: ... Proceed?」
- 右键菜单 5 项 + 复制 toast 标签（Path / Absolute path / Relative path / File name）
- iframe 加载失败 fallback 卡（标题 + 重试 / 关闭 + 4 条原因）
- 扫描根行的开关 tooltip（Enable / Disable scanning） + 移除按钮
- 主题切换 toast 文案 + tooltip 三态（auto / light / dark）
- 历史相对时间长短两套（「3 minutes ago」长形式 / 「3m ago」短形式）
- 浏览选择按钮「✓ Select / Choose this directory」
- 清理 / 稀释结果消息「Scanned X dirs, removed Y...」「X dirs · removed Y/Z · kept K」
- 错误提示「Response is not JSON」「JSON parse failed」「server.py may need restart」

**关键能力**：切换语言瞬间，所有已打开的面板（时光机、历史抽屉、文件夹浏览、设置、快捷键）**立即重译**，无需刷新。
...
| 问题 | 根因 | 解法 |
|---|---|---|
| 快捷键面板切英文仍中文 | 它是 `document.createElement` + `innerHTML` 字符串拼接懒创建的 DOM，首次 `applyDOM()` 扫描时还不存在；切换语言时也没重渲染 | (1) HTML 字符串模板内全部加 `data-i18n` 标记 + 英文 fallback；(2) `document.body.appendChild` 后立即 `window.i18n.applyDOM(el)` 把刚注入的子树扫一遍 |
| 切换语言后已打开的时光机 / 最近 / 收藏不切换 | langchange 事件只触发了顶层 `applyDOM()`，但这些列表的标题/按钮文案是 JS 字符串拼出来后 set innerHTML 的，没有 data-i18n 标记 | langSwitcherCtl.init 监听 `langchange` 全局事件，重新调用 `renderFavoritesSection() / renderRecentSection() / renderTree(state.tree)` 重新生成 |
| 我之前批量替换脚本 `title=window.i18n.t(...)` 没引号 | 替换规则没考虑 HTML 属性值必须有引号；浏览器把它当成 attribute 名 | 全部改为 `title="${escapeHtml(window.i18n.t(...))}"` 模式，并 grep `(title\|placeholder\|alt\|aria-label)=window\.` 校验零残留 |

### 🔧 改动统计

| 维度 | v1.12.0 | v1.12.1 | 增量 |
|---|---|---|---|
| 字典 keys (en/zh) | 215 × 2 | 304 × 2 | +89 × 2 |
| `data-i18n*` 标记数 | 83 | 96+ | +13（动态懒创建 DOM）|
| `window.i18n.t()` 调用点 | 93 | 176 | +83 |
| 涉及不同 keys | 80 | 157 | +77 |
...
| 文件 | 改动 |
|---|---|
| `web/app.js` | 快捷键面板 + 时光机 + 历史项 + iframe fallback + 收藏 / 最近 / 树空态 / 拖拽 confirm / 右键菜单 / 主题 / 错误提示等 13 个区域 i18n 化；新增 langchange 监听重渲染逻辑 |
| `web/locales/en.js` | +89 keys（time / recent / fav / theme / tree / move / menu / iframe / scan / browse / history.kind / tm / cleanup / err / bc）|
| `web/locales/zh.js` | +89 keys 对应中文 |
| `CHANGELOG.md` | 新增 v1.12.1 卡片 |

### 🤔 设计决策

- **为什么动态 DOM 注入后立即调 `applyDOM(el)`？** 比强制要求"所有 DOM 都注册到全局 i18n 引擎"侵入性低。子树范围扫描，性能开销可忽略。
- **为什么不让 langchange 直接 `applyDOM(document)` 就够了？** `data-i18n` 是声明式翻译，但 `renderXxx()` 函数内部的 `\`已收藏 ${count}\`` 这类 JS 拼接字符串不挂 `data-i18n` 标记，只能靠重渲染。重渲染 + applyDOM 双轨制最稳妥。
- **为什么把短形式相对时间（time.minutes_ago_short）单独建 key？** 历史抽屉空间紧凑要短；最近列表 tooltip 要长。设计上不强行复用，让 EN 自由用「3m ago / 3 minutes ago」两种风格。
- **为什么 v1.12.1 而不是合进 v1.12.0？** v1.12.0 已 commit 但未 push，技术上可以 amend；但保留 v1.12.0 的"框架完成"vs v1.12.1 的"全面覆盖"两个阶段，对未来读者更清晰，避免单 commit 跨越「设计 + 收尾」两个心智阶段。

### ✅ 验证（铁律 1）

- [x] `node --check` 全部 4 个 JS 文件语法 OK
- [x] EN 304 keys、ZH 304 keys、`en ^ zh = ∅`
- [x] app.js 176 处 `i18n.t()` 调用、157 unique keys，全部存在于字典
- [x] `(title\|placeholder\|alt\|aria-label)=window\.` 语法错误 = 0
- [x] 残留中文调用点 = 1 处（L107 是注释）
- [ ] **真实浏览器演练待 Louis 进行**：⌨️ 快捷键面板 / 时光机 / 历史抽屉 / 拖拽移动 / iframe 加载失败 / 右键菜单 / 主题切换 / 设置面板各种动作
